...
def show_result(num_epoch, show = False, save = False, path = 'result.png'):
    z_ = np.random.normal(0, 1, (25, 128))    # z_ is the input of generator, every epochs will random produce input
...
def saveimage():
    ...
    for i in range (batch_size):
        ax[i].set_axis_off()
        ax[i].imshow(np.reshape(loss_g_[i], (128, 128)))
...
for epoch in range(train_epoch):
    ...
    for iter in range(train_set.shape[0] // batch_size):
        ...
        z_ = np.random.normal(0, 1, (batch_size, 128))
